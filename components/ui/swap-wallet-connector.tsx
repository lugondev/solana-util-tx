...
export function SwapWalletConnector({quote, loadingSwap, onExecuteSwap, transactionSignature, className = ''}: SwapWalletConnectorProps) {
				...
				<div className='space-y-4'>
					...
					<div className='p-4 bg-red-900/20 border-2 border-red-600/30'>
						<div className='flex items-start gap-2'>
							...
							<div className='font-mono text-xs text-red-400'>MetaMask Solana Snap is currently blocked due to critical stability issues that cause transaction failures.</div>
						</div>
					</div>
					...
					<div className='text-center'>
						<PixelButton onClick={() => window.open('https://phantom.app/', '_blank')} variant='secondary' className='w-full !bg-purple-600 !border-purple-400 !text-white'>
							[DOWNLOAD PHANTOM]
						</PixelButton>
					</div>
				</div>
				...
				<div className='space-y-4'>
					...
					<div className='p-4 bg-yellow-900/20 border-2 border-yellow-600/30'>
						<div className='flex items-start gap-2'>
							...
							<div className='font-mono text-xs text-yellow-400'>Connect your wallet to execute swaps</div>
						</div>
					</div>
			...
			<div className='space-y-4'>
				...
				<div className='p-3 bg-green-900/20 border-2 border-green-600/30'>
					<div className='font-mono text-xs text-green-400 space-y-1'>
						...
						<div className='text-xs text-blue-400 mt-1'>{wallet?.adapter?.name || 'Unknown Wallet'}</div>
						{/* Debug info */}
				...
				{isMetaMask && (
					<div className='p-3 bg-yellow-900/20 border-2 border-yellow-600/30'>
						<div className='flex items-start gap-2'>
							<AlertCircle className='h-4 w-4 text-yellow-400 mt-0.5' />
							<div className='font-mono text-xs text-yellow-400'>⚠️ MetaMask Solana Snap can be unstable. If swap fails, try refreshing or use Phantom wallet for better reliability.</div>
						</div>
					</div>
				)}
				...
				{transactionSignature && (
					<div className='p-3 bg-blue-900/20 border-2 border-blue-600/30'>
						<div className='font-mono text-xs text-blue-400 space-y-2'>
							...
							<PixelButton onClick={() => window.open(getExplorerUrl(transactionSignature), '_blank')} variant='secondary' className='w-full !text-xs !py-1'>
				...
				<div className='space-y-3'>
						...
						<div className='p-3 bg-green-900/20 border-2 border-green-600/30'>
							<div className='font-mono text-xs text-green-400 space-y-2'>
								...
								<div className='flex justify-between'>
									...
									<span className={parseFloat(quote.priceImpactPct) <= 1 ? 'text-green-400' : 'text-red-400'}>{parseFloat(quote.priceImpactPct).toFixed(2)}%</span>
								</div>
							</div>
						</div>
					) : (
						<div className='p-3 bg-blue-900/20 border-2 border-blue-600/30'>
							<div className='font-mono text-xs text-blue-400 text-center'>ℹ️ Enter token details and get a quote first, then you can execute swap here</div>
						</div>
					)}

					{/* Always show swap button when wallet connected */}
					<PixelButton onClick={handleExecuteSwap} disabled={!quote || loadingSwap || connecting} className={`w-full ${!quote ? '!bg-gray-600 !text-gray-300 !border-gray-500' : '!bg-green-400 !text-gray-900 !border-green-400'}`}>
						{loadingSwap ? (
							<>
					...
					<div className='p-2 bg-gray-800 border border-gray-600'>
						<div className='font-mono text-xs text-gray-400 text-center'>{!quote ? 'Enter token details → Get Quote → Execute Swap' : `Ready to swap ${formatTokenAmount(quote.inAmount, inputTokenData.decimals, 2)} ${inputTokenData.symbol} → ${formatTokenAmount(quote.outAmount, outputTokenData.decimals, 2)} ${outputTokenData.symbol}`}</div>
					</div>

					{quote && (
						<div className={`p-3 border-2 ${loadingSwap ? 'bg-blue-900/20 border-blue-600/30' : 'bg-yellow-900/20 border-yellow-600/30'}`}>
							<div className='flex items-start gap-2'>
								{loadingSwap ? <RefreshCw className='h-4 w-4 mt-0.5 text-blue-400 animate-spin flex-shrink-0' /> : <AlertCircle className='h-4 w-4 mt-0.5 text-yellow-400 flex-shrink-0' />}
								<div className={`font-mono text-xs ${loadingSwap ? 'text-blue-400' : 'text-yellow-400'}`}>{loadingSwap ? 'Please confirm the transaction in your wallet popup. This may take a moment...' : isMetaMask ? 'MetaMask detected. If transaction fails, try refreshing page or reconnecting wallet.' : 'Review transaction details before signing. Swaps are irreversible.'}</div>
							</div>
						</div>
					)}
				</div>
