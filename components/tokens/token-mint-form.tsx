...
import { 
  createTokenAndMint,
  TOKEN_PRESETS,
  type CreateTokenAndMintParams 
} from '@/lib/solana/tokens/spl-token/mint'
...
export function TokenMintForm() {
  ...
  const handleMintToken = async () => {
    ...
    try {
      ...
      const { transaction, mintKeypair, mintAddress, estimatedCost } = await createTokenAndMint(params)
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {Object.entries(TOKEN_PRESETS).map(([key, preset]) => (
                <button
                  key={key}
                  onClick={() => handlePresetSelect(key as keyof typeof TOKEN_PRESETS)}
                  disabled={isMinting}
                  className={`p-2 border-4 transition-colors font-pixel text-xs ${
                    selectedPreset === key
                      ? 'border-green-400 bg-green-400/10 text-green-400'
                      : 'border-gray-700 hover:border-green-400/50 text-gray-400'
                  }`}
                >
          ...
          <div className="p-3 bg-gray-800 border-4 border-gray-700">
            ...
            <div className="font-mono text-xs text-gray-500 mt-1">
              With {formData.decimals} decimal places
            </div>
          </div>
        </div>
      </PixelCard>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 font-pixel text-xs text-gray-400">
                <input
                  type="radio"
                  checked={!formData.useCustomRecipient}
                  onChange={() => handleInputChange('useCustomRecipient', false)}
                  disabled={isMinting}
                  className="w-4 h-4"
                />
                MINT TO SELF
              ...
              <label className="flex items-center gap-2 font-pixel text-xs text-gray-400">
                <input
                  type="radio"
                  checked={formData.useCustomRecipient}
                  onChange={() => handleInputChange('useCustomRecipient', true)}
                  disabled={isMinting}
                  className="w-4 h-4"
                />
                CUSTOM RECIPIENT
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="font-mono text-xs text-green-400 break-all p-2 bg-gray-800 border-4 border-gray-700 flex-1">
                    {result.mintAddress}
                  </div>
                  <button
                  ...
                  <button
                    onClick={() => openInExplorer(result.mintAddress)}
                    className="text-gray-400 hover:text-green-400 p-2"
                  >
              ...
              <div>
                ...
                <div className="font-mono text-xs text-green-400 p-2 bg-gray-800 border-4 border-gray-700">
                  {result.estimatedCost.toFixed(6)} SOL
                </div>
              </div>

              <div className="p-3 bg-green-400/10 border-4 border-green-400/20">
                <div className="font-mono text-xs text-green-400">
                  🎉 Your token has been created! You can now transfer, mint more, or burn tokens using the token operations.
                  Save the mint address to interact with your token later.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 font-mono text-xs text-gray-400">
            <div className="flex items-start gap-2">
              ...
              <span>
                Creates a new SPL token mint with you as the mint authority
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                Decimals determine the smallest unit (e.g., 9 decimals = divisible by 1,000,000,000)
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                Initial supply will be minted to the specified recipient
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                As mint authority, you can mint more tokens later or transfer/revoke the authority
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                Cost includes rent for mint account (~0.00144 SOL) plus transaction fees
              </span>
            </div>
          </div>
        </div>
