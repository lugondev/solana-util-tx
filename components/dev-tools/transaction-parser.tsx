          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder="Enter base58 encoded transaction or signature..."
                className="w-full h-32 bg-gray-800 border-4 border-gray-600 font-mono text-sm text-white p-4 resize-none focus:border-green-400 focus:outline-none"
                disabled={loading}
              />
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {parsedTx && (
                <>
                  <PixelButton
                    variant="secondary"
                    onClick={() => handleCopy(TransactionParser.formatParsedTransaction(parsedTx))}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {Object.entries(transactionExamples).map(([key, example]) => (
                  <button
                    ...
                    disabled={loading}
                    className="p-3 bg-gray-800 border-2 border-gray-700 hover:border-blue-500 transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed"
                  >
          ...
          <PixelCard>
            <div className="space-y-4">
              <div className="border-b-4 border-blue-400/20 pb-4">
                <h3 className="font-pixel text-lg text-blue-400 mb-2">
                  📋 TRANSACTION OVERVIEW
                </h3>
              </div>
                ...
                {parsedTx.priorityFee && (
                  <div className="p-4 bg-gray-800 border-4 border-gray-700">
                    ...
                    <div className="font-pixel text-sm text-yellow-400">
                      {parsedTx.priorityFee.toFixed(6)} SOL
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="font-mono text-sm text-yellow-400">
                      {parsedTx.computeUnitLimit.toLocaleString()}
                    </div>
                  </div>
                  {parsedTx.computeUnitPrice && (
                    <div>
                      ...
                      <div className="font-mono text-sm text-yellow-400">
                        {parsedTx.computeUnitPrice} μLamports
                      </div>
                    </div>
                  )}
                </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className={`font-mono text-xs ${getInstructionColor(instruction.programName)}`}>
                          ...
                          <div className="space-y-2">
                            {Object.entries(instruction.data).map(([key, value]) => (
                              <div key={key} className="flex justify-between items-start gap-4">
                                <span className="font-mono text-xs text-blue-400 min-w-0 flex-shrink-0">
                                  {key}:
                                </span>
                                <span className="font-mono text-xs text-white break-all text-right">
                                  {key.includes('amount') && typeof value === 'string' && value.match(/^\d+$/) 
                                    ? formatAmount(value)
                                    : String(value)
                                  }
                                </span>
                          ...
                          <div className="space-y-1">
                            {instruction.accounts.map((account, i) => (
                              <div key={i} className="flex items-start gap-2">
                                <span className="font-mono text-xs text-gray-500 min-w-0 flex-shrink-0">
                                  {i}:
                                </span>
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="flex gap-2 mt-1">
                                    {account.isSigner && (
                                      <span className="px-2 py-0.5 bg-blue-600/20 border border-blue-600/30 font-mono text-xs text-blue-300">
                                        signer
                                      </span>
                                    )}
                                    {account.isWritable && (
                                      <span className="px-2 py-0.5 bg-yellow-600/20 border border-yellow-600/30 font-mono text-xs text-yellow-300">
                                        writable
                                      </span>
                                    )}
                                  </div>
            ...
            <PixelCard>
              <div className="space-y-4">
                <div className="border-b-4 border-purple-400/20 pb-4">
                  <h3 className="font-pixel text-lg text-purple-400 mb-2">
                    📋 ADDRESS LOOKUP TABLES
                  </h3>
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <div className="font-mono text-xs text-gray-400 mb-1">
                              Writable Indexes ({alt.writableIndexes.length}):
                            </div>
                            <div className="font-mono text-xs text-yellow-400">
                              [{alt.writableIndexes.join(', ')}]
                            </div>
                          </div>
                          
                          <div>
                            <div className="font-mono text-xs text-gray-400 mb-1">
                              Readonly Indexes ({alt.readonlyIndexes.length}):
                            </div>
                            <div className="font-mono text-xs text-blue-400">
                              [{alt.readonlyIndexes.join(', ')}]
                            </div>
                          </div>
                        </div>
                      </div>
