...
export function NetworkSwitcher() {
	...
	return (
		<div className='relative'>
			<PixelButton variant={network === 'mainnet-beta' ? 'danger' : 'secondary'} size='sm' onClick={() => setIsOpen(!isOpen)}>
			...
			{isOpen && (
				<>
					...
					{/* Dropdown */}
					<div
						className='absolute top-full left-0 mt-2 z-20 border-4 min-w-48'
						style={{
						...
						<div className='p-2 space-y-1'>
							...
							{showCustomInput ? (
								<div className='px-3 py-2 space-y-2'>
									<div>
										<label htmlFor='custom-rpc' className='block font-mono text-xs mb-1' style={{color: 'var(--pixel-text-primary)'}}>
											Custom RPC URL
										</label>
										<input
											id='custom-rpc'
									...
									{Object.entries(NETWORKS).map(([key, config]) => {
										...
										const getDescription = () => {
											switch (key) {
												...
												case 'custom':
													return customRpcUrl || 'Your custom RPC endpoint'
												default:
													return 'Development network'
											}
										}
										...
										return (
											<button
												...
												<div className='flex items-center justify-between'>
													...
													{network === key && <span style={{color: 'var(--pixel-bg-primary)'}}>✓</span>}
												</div>
												<div className='text-xs mt-1 opacity-75 truncate'>{getDescription()}</div>
											</button>
										)
									})}
								</>
