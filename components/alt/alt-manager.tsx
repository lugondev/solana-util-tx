...
import { ChevronDown, ChevronRight, ExternalLink, Copy, Trash2 } from 'lucide-react'
      ...
      <PixelCard>
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <PixelInput
              value={newALTAddress}
              onChange={(e) => setNewALTAddress(e.target.value)}
              placeholder="Enter ALT address to track"
              disabled={isAdding}
              className="flex-1"
            />
            <PixelButton
              ...
              disabled={!newALTAddress.trim() || isAdding}
              isLoading={isAdding}
            >
              [ADD]
            </PixelButton>
      ...
      <PixelCard>
        <div className="space-y-4">
          <div className="border-b-4 border-green-400/20 pb-3 flex items-center justify-between">
            <h3 className="font-pixel text-sm text-green-400">
              MANAGED ALTS ({storedALTs.length})
            </h3>
            <PixelButton
              variant="secondary"
              onClick={loadALTInfos}
              disabled={isLoading}
              isLoading={isLoading}
          ...
          {storedALTs.length === 0 ? (
            <div className="text-center py-8">
              ...
              <div className="font-mono text-xs text-gray-600 mt-2">
                Create a new ALT or add an existing one to start managing
              </div>
            </div>
          ) : (
                    ...
                    <div className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => toggleExpanded(alt.address)}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => openInExplorer(alt.address)}
                            className="text-gray-400 hover:text-green-400 p-1"
                          ...
                          <button
                            onClick={() => handleRemoveALT(alt.address)}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="font-mono text-xs text-white break-all p-2 bg-gray-900 border-2 border-gray-600">
                                {altInfo.authority?.toBase58() || 'None (Frozen)'}
                              </div>
                            </div>
                            <div>
                              <div className="font-mono text-xs text-gray-400 mb-1">ADDRESSES:</div>
                              <div className="font-mono text-xs text-white p-2 bg-gray-900 border-2 border-gray-600">
                          ...
                          {alt.description && (
                            <div>
                              ...
                              <div className="font-mono text-xs text-gray-300 p-2 bg-gray-900 border-2 border-gray-600">
                                {alt.description}
                              </div>
                            </div>
                          )}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 font-mono text-xs text-gray-400">
            <div className="flex items-start gap-2">
              ...
              <span>
                Track and manage your Address Lookup Tables from one place
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                View all addresses stored in each ALT and their indices
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                Monitor ALT status: active, deactivated, frozen, or closable
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-400 mt-0.5">▸</span>
              <span>
                Only the authority can extend, deactivate, or close an ALT
              </span>
            </div>
          </div>
        </div>
