	...
	return (
		<div className='container mx-auto px-4 py-8 max-w-6xl'>
			<div className='mb-8'>
				...
				<p className='font-mono text-sm text-gray-400'>View, extend, and manage existing Address Lookup Tables</p>
			</div>
				...
				<div className='space-y-6'>
					<PixelCard>
						<div className='space-y-4'>
							<div className='border-b-4 border-green-400/20 pb-3'>
								<h3 className='font-pixel text-sm text-green-400'>🔍 SEARCH ALT</h3>
							</div>

							<div className='flex gap-3'>
								<div className='flex-1'>
									<PixelInput label='ALT ADDRESS' value={altAddress} onChange={(e) => setAltAddress(e.target.value)} placeholder='Enter Address Lookup Table address' />
								</div>
								<div className='flex items-end'>
									<PixelButton onClick={handleSearchALT} disabled={loading} className='h-12'>
					...
					{altInfo && (
						<PixelCard>
							<div className='space-y-4'>
								<div className='border-b-4 border-green-400/20 pb-3'>
									<h3 className='font-pixel text-sm text-green-400'>📊 ALT INFORMATION</h3>
								</div>
								...
								<div className='space-y-3'>
									<div className='grid grid-cols-2 gap-4'>
										...
										<div className='text-center p-3 bg-gray-800 border-2 border-gray-700'>
											<div className={`font-mono text-lg ${altInfo.isActive ? 'text-green-400' : 'text-red-400'}`}>{altInfo.isActive ? 'ACTIVE' : 'INACTIVE'}</div>
									...
									<div className='space-y-2'>
										<div className='flex justify-between'>
											...
											<div className='flex items-center gap-2'>
												<span className='font-mono text-xs text-white'>{altInfo.authority ? `${altInfo.authority.toString().slice(0, 6)}...${altInfo.authority.toString().slice(-4)}` : 'FROZEN'}</span>
												{altInfo.authority && <Eye className='h-3 w-3 text-gray-400 cursor-pointer hover:text-green-400' />}
											</div>
										</div>
										<div className='flex justify-between'>
											<span className='font-mono text-xs text-gray-400'>Size Optimization:</span>
											<span className='font-mono text-xs text-green-400'>~{Math.max(0, (altInfo.addresses.length - 1) * 32)} bytes saved</span>
										</div>
									</div>
								</div>
							</div>
				...
				<div className='space-y-6'>
					{altInfo && (
						<PixelCard>
							<div className='space-y-4'>
								<div className='border-b-4 border-green-400/20 pb-3'>
									<h3 className='font-pixel text-sm text-green-400'>📋 ADDRESSES ({altInfo.addresses.length})</h3>
								</div>

								<div className='max-h-96 overflow-y-auto border-2 border-gray-700 bg-gray-800'>
									{altInfo.addresses.map((address, index) => (
										<div key={index} className='px-3 py-2 border-b border-gray-700 last:border-b-0 flex items-center justify-between'>
											<div className='font-mono text-xs text-white'>
												<span className='text-gray-500 mr-2'>{index}:</span>
												{address.toString()}
											</div>
											<ExternalLink className='h-3 w-3 text-gray-400 cursor-pointer hover:text-green-400' onClick={() => window.open(`https://explorer.solana.com/address/${address.toString()}`, '_blank')} />
										</div>
									))}
									{altInfo.addresses.length === 0 && <div className='px-3 py-4 text-center font-mono text-xs text-gray-500'>No addresses in this ALT</div>}
								</div>
							</div>
						</PixelCard>
					)}
					...
					{altInfo && (
						<PixelCard>
							<div className='space-y-4'>
								<div className='border-b-4 border-green-400/20 pb-3'>
									<h3 className='font-pixel text-sm text-green-400'>📈 USAGE STATISTICS</h3>
								</div>

								<div className='grid grid-cols-2 gap-4'>
									<div className='text-center p-3 bg-gray-800 border-2 border-gray-700'>
										<div className='font-mono text-lg text-green-400'>{Math.max(0, (altInfo.addresses.length - 1) * 32)}B</div>
										<div className='font-mono text-xs text-gray-400'>Size Saved</div>
									</div>
									<div className='text-center p-3 bg-gray-800 border-2 border-gray-700'>
										<div className='font-mono text-lg text-blue-400'>~{(altInfo.addresses.length * 0.000005).toFixed(6)}</div>
										<div className='font-mono text-xs text-gray-400'>SOL Fee Saved</div>
									</div>
								</div>

								<div className='pt-3 border-t-4 border-gray-700'>
									<div className='font-mono text-xs text-gray-400 text-center'>Estimated savings per transaction using this ALT</div>
								</div>
							</div>
						</PixelCard>
					)}
			...
			{toast && <PixelToast message={toast.message} type={toast.type} onClose={() => setToast(null)} />}
		</div>
	)
}
