...
import {Key, Code, Rocket, Settings, Search, Eye, Hash, Wallet, FileText, Database, Zap, Brain, Wrench, Activity, Terminal} from 'lucide-react'
...
export default function DevToolsPage() {
	const tools = [
		...
		},
		{
			...
			description: 'Import IDL and interact with Solana programs',
			href: '/dev-tools/program-interaction',
			icon: Brain,
			status: 'active',
			category: 'Program Tools',
			features: ['IDL import', 'Auto-generated forms', 'Type validation', 'Execute instructions'],
		},
		{
	...
	return (
		<div className='container mx-auto px-4 py-8 max-w-7xl'>
			...
			<div className='mb-12 text-center'>
				...
				<p className='font-mono text-lg text-gray-400 mb-6'>Comprehensive Solana development utilities and tools</p>

				{/* Stats */}
				<div className='grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto'>
					<div className='bg-gray-800 border-4 border-green-400/20 p-4'>
						<div className='font-pixel text-2xl text-green-400'>{tools.filter((t) => t.status === 'active').length}</div>
						<div className='font-mono text-xs text-gray-400'>Ready</div>
					</div>
					<div className='bg-gray-800 border-4 border-yellow-400/20 p-4'>
						<div className='font-pixel text-2xl text-yellow-400'>{tools.filter((t) => t.status === 'beta').length}</div>
						<div className='font-mono text-xs text-gray-400'>Beta</div>
					</div>
					<div className='bg-gray-800 border-4 border-blue-400/20 p-4'>
						<div className='font-pixel text-2xl text-blue-400'>{tools.filter((t) => t.status === 'coming-soon').length}</div>
										...
										<div className='flex flex-col h-full'>
											<div className='flex items-start justify-between mb-4'>
												<div className='flex items-center gap-3'>
													...
													<div>
														<h3 className='font-pixel text-sm text-white mb-1'>{tool.title}</h3>
														{getStatusBadge(tool.status)}
													</div>
												</div>
											</div>

											<p className='font-mono text-sm text-gray-400 mb-4 flex-1'>{tool.description}</p>
											...
											{tool.status === 'active' || tool.status === 'beta' ? (
												<Link href={tool.href}>
													<PixelButton variant={tool.status === 'active' ? 'primary' : 'secondary'} className='w-full !text-xs'>
				...
				<PixelCard>
					<div className='space-y-6'>
						<div className='border-b-4 border-purple-400/20 pb-4'>
							<h2 className='font-pixel text-xl text-purple-400 mb-2'>🗺️ DEVELOPMENT ROADMAP</h2>
							<p className='font-mono text-sm text-gray-400'>Upcoming tools and features in the development pipeline</p>
						</div>
							...
							<div className='p-4 bg-purple-900/20 border-2 border-purple-600/30'>
								<div className='flex items-start gap-3'>
									...
									<div className='space-y-2'>
										...
										<div className='font-mono text-xs text-gray-300'>These tools are designed to be comprehensive and developer-friendly. If you have suggestions for new tools or improvements to existing ones, feedback is always welcome to make this the ultimate Solana dev toolkit.</div>
									</div>
								</div>
							</div>
						</div>
