...
export default function TokenAnalysisPage() {
	...
	const handleAnalyze = async () => {
		...
		try {
			...
			setError(error instanceof Error ? error.message : 'Error analyzing token')
		} finally {
			setLoading(false)
		}
	}
	...
	return (
		<div className='container mx-auto px-4 py-8 max-w-6xl'>
			...
			<div className='mb-8'>
				...
				<p className='font-mono text-sm text-gray-400'>Analyze SPL token metrics, holder distribution, and market data</p>
			</div>
			...
			<div className='space-y-6'>
				...
				<PixelCard>
					<div className='space-y-4'>
						<div className='border-b-4 border-green-400/20 pb-3'>
							<h3 className='font-pixel text-sm text-green-400'>🔍 TOKEN SEARCH</h3>
						</div>

						<div className='flex gap-3'>
							<div className='flex-1'>
								<PixelInput label='TOKEN MINT ADDRESS' value={tokenAddress} onChange={(e) => setTokenAddress(e.target.value)} placeholder='Enter SPL token mint address' />
							</div>
							<div className='flex items-end'>
								<PixelButton onClick={handleAnalyze} disabled={loading} className='h-12'>
									{loading ? (
										<>
					...
					<>
						<div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
							...
							<PixelCard>
								<div className='space-y-4'>
									<div className='border-b-4 border-green-400/20 pb-3'>
										<h3 className='font-pixel text-sm text-green-400'>📊 BASIC INFO</h3>
									</div>

									<div className='space-y-3'>
										<div className='flex items-center gap-3'>
											<div className='w-8 h-8 bg-blue-600 border-2 border-blue-400 flex items-center justify-center'>
												<span className='font-pixel text-xs text-white'>{tokenInfo.symbol.slice(0, 2)}</span>
											</div>
											<div>
												<div className='font-pixel text-sm text-white'>{tokenInfo.name}</div>
												<div className='font-mono text-xs text-gray-400'>${tokenInfo.symbol}</div>
											</div>
											{tokenInfo.verified && <div className='ml-auto w-2 h-2 bg-green-400 rounded-full'></div>}
										</div>
										...
										<div className='space-y-2'>
											<div className='flex justify-between'>
												...
												<div className='flex items-center gap-2'>
													<span className='font-mono text-xs text-white'>{`${tokenInfo.address.slice(0, 6)}...${tokenInfo.address.slice(-4)}`}</span>
							...
							<PixelCard>
								<div className='space-y-4'>
									<div className='border-b-4 border-green-400/20 pb-3'>
										<h3 className='font-pixel text-sm text-green-400'>💰 PRICE DATA</h3>
									</div>

									<div className='space-y-3'>
										<div className='text-center'>
											<div className='font-pixel text-2xl text-white'>${tokenInfo.price?.toFixed(4)}</div>
											<div className={`font-mono text-sm flex items-center justify-center gap-1 ${(tokenInfo.change24h || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
												<TrendingUp className='h-4 w-4' />
												{(tokenInfo.change24h || 0) >= 0 ? '+' : ''}
												{tokenInfo.change24h?.toFixed(2)}%
											</div>
										</div>
										...
										<div className='space-y-2'>
											<div className='flex justify-between'>
												...
												<span className='font-mono text-xs text-white'>${formatNumber(tokenInfo.volume24h || 0)}</span>
											</div>
											<div className='flex justify-between'>
												<span className='font-mono text-xs text-gray-400'>Market Cap:</span>
												<span className='font-mono text-xs text-white'>${formatNumber((tokenInfo.price || 0) * parseFloat(tokenInfo.supply.replace(/,/g, '')))}</span>
											</div>
										</div>
									</div>
								</div>
							...
							<PixelCard>
								<div className='space-y-4'>
									<div className='border-b-4 border-green-400/20 pb-3'>
										<h3 className='font-pixel text-sm text-green-400'>👥 HOLDER ANALYTICS</h3>
									</div>

									<div className='space-y-3'>
										<div className='text-center'>
											<div className='font-pixel text-2xl text-white'>{formatNumber(tokenInfo.holders || 0)}</div>
											<div className='font-mono text-xs text-gray-400'>Total Holders</div>
										</div>
										...
										{analyticsData && (
											<div className='space-y-2'>
												<div className='flex justify-between'>
													...
													<span className='font-mono text-xs text-white'>{analyticsData.distribution.top10Percentage.toFixed(1)}%</span>
												</div>
												<div className='flex justify-between'>
													<span className='font-mono text-xs text-gray-400'>Top 100 Hold:</span>
													<span className='font-mono text-xs text-white'>{analyticsData.distribution.top100Percentage.toFixed(1)}%</span>
												</div>
												<div className='flex justify-between'>
													<span className='font-mono text-xs text-gray-400'>Distribution:</span>
													<span className={`font-mono text-xs ${analyticsData.distribution.concentrationRisk === 'HIGH' ? 'text-red-400' : analyticsData.distribution.concentrationRisk === 'MEDIUM' ? 'text-yellow-400' : 'text-green-400'}`}>{analyticsData.distribution.concentrationRisk}</span>
												</div>
												<div className='flex justify-between'>
													<span className='font-mono text-xs text-gray-400'>Gini Coefficient:</span>
													<span className='font-mono text-xs text-white'>{analyticsData.distribution.giniCoefficient.toFixed(3)}</span>
												</div>
											</div>
										)}
									</div>
						...
						<div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
							...
							<PixelCard>
								<div className='space-y-4'>
									<div className='border-b-4 border-green-400/20 pb-3'>
										<h3 className='font-pixel text-sm text-green-400'>📈 PRICE CHART (24H)</h3>
									</div>
									...
									<div className='h-48 flex items-center justify-center border-4 border-gray-700'>
										<div className='text-center'>
											...
											<div className='font-pixel text-sm text-gray-500'>CHART COMING SOON</div>
											<div className='font-mono text-xs text-gray-600'>Integration with TradingView</div>
										</div>
									</div>
								</div>
							</PixelCard>
							...
							<PixelCard>
								<div className='space-y-4'>
									<div className='border-b-4 border-green-400/20 pb-3'>
										<h3 className='font-pixel text-sm text-green-400'>📊 HOLDER DISTRIBUTION</h3>
									</div>
									...
									{analyticsData ? (
										<div className='space-y-3'>
											<div className='space-y-2'>
												...
												<div className='w-full bg-gray-700 h-2 border-2 border-gray-600'>
													<div className='bg-red-500 h-full' style={{width: `${analyticsData.distribution.top10Percentage}%`}}></div>
												</div>
											</div>
											...
											<div className='space-y-2'>
												<div className='flex justify-between items-center'>
													...
													<span className='font-mono text-xs text-white'>{(analyticsData.distribution.top100Percentage - analyticsData.distribution.top10Percentage).toFixed(1)}%</span>
												</div>
												<div className='w-full bg-gray-700 h-2 border-2 border-gray-600'>
													<div className='bg-yellow-500 h-full' style={{width: `${analyticsData.distribution.top100Percentage - analyticsData.distribution.top10Percentage}%`}}></div>
												</div>
											</div>
											...
											<div className='space-y-2'>
												<div className='flex justify-between items-center'>
													...
													<span className='font-mono text-xs text-white'>{(100 - analyticsData.distribution.top100Percentage).toFixed(1)}%</span>
												</div>
												<div className='w-full bg-gray-700 h-2 border-2 border-gray-600'>
													<div className='bg-green-500 h-full' style={{width: `${100 - analyticsData.distribution.top100Percentage}%`}}></div>
												</div>
											</div>
											...
											<div className='pt-3 border-t-4 border-gray-700'>
												<div className='flex items-center gap-2'>
													...
													<span className='font-mono text-xs text-yellow-400'>Concentration Risk: {analyticsData.distribution.concentrationRisk}</span>
												</div>
											</div>
										</div>
									) : (
										<div className='space-y-3'>
											...
											<div className='pt-3 border-t-4 border-gray-700'>
												<div className='flex items-center gap-2'>
													<Activity className='h-4 w-4 text-yellow-400' />
													<span className='font-mono text-xs text-yellow-400'>Concentration Risk: MEDIUM</span>
												</div>
											</div>
										</div>
									)}
						...
						<PixelCard>
							<div className='space-y-4'>
								<div className='border-b-4 border-green-400/20 pb-3'>
									<h3 className='font-pixel text-sm text-green-400'>🔬 ADVANCED ANALYTICS</h3>
								</div>
									...
									<>
										<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
											<div className='text-center p-4 bg-gray-800 border-2 border-gray-700'>
												<div className='font-mono text-lg text-green-400'>{analyticsData.liquidity.liquidityScore.toFixed(1)}%</div>
												<div className='font-mono text-xs text-gray-400'>Liquidity Score</div>
											</div>
											<div className='text-center p-4 bg-gray-800 border-2 border-gray-700'>
												<div className='font-mono text-lg text-blue-400'>{analyticsData.risk.overallScore.toFixed(1)}</div>
												<div className='font-mono text-xs text-gray-400'>Risk Rating</div>
											</div>
											<div className='text-center p-4 bg-gray-800 border-2 border-gray-700'>
												<div className='font-mono text-lg text-purple-400'>{formatNumber(analyticsData.activity.transactions24h)}</div>
												<div className='font-mono text-xs text-gray-400'>24h Transactions</div>
											</div>
											<div className='text-center p-4 bg-gray-800 border-2 border-gray-700'>
												<div className='font-mono text-lg text-yellow-400'>{(analyticsData.activity.volatility24h * 100).toFixed(1)}%</div>
											...
											<div className='p-4 bg-gray-800 border-2 border-gray-700'>
												...
												<div className='space-y-1'>
													<div className='flex justify-between'>
														...
														<span className='font-mono text-xs text-white'>{formatNumber(analyticsData.liquidity.dexLiquidity)}</span>
													</div>
													<div className='flex justify-between'>
														<span className='font-mono text-xs text-gray-400'>Market Cap:</span>
														<span className='font-mono text-xs text-white'>${formatNumber(analyticsData.liquidity.marketCap)}</span>
													</div>
													<div className='flex justify-between'>
														<span className='font-mono text-xs text-gray-400'>Slippage (1K):</span>
														<span className='font-mono text-xs text-white'>{analyticsData.liquidity.slippageEstimate.buy1000.toFixed(2)}%</span>
													</div>
												</div>
											</div>
												...
												<div className='space-y-1'>
													{analyticsData.risk.factors.slice(0, 3).map((factor, idx) => (
														<div key={idx} className='flex justify-between'>
															...
															<span className={`font-mono text-xs ${factor.score > 7 ? 'text-red-400' : factor.score > 4 ? 'text-yellow-400' : 'text-green-400'}`}>{factor.score.toFixed(1)}/10</span>
														</div>
													))}
												</div>
													...
													{analyticsData.largestHolders.slice(0, 5).map((holder, idx) => (
														<div key={idx} className='flex items-center justify-between'>
															<div className='flex items-center gap-2'>
																...
																{holder.isContract && <span className='font-pixel text-xs text-blue-400'>[CONTRACT]</span>}
															</div>
															<div className='text-right'>
																<div className='font-mono text-xs text-white'>{formatNumber(holder.balance)}</div>
																<div className='font-mono text-xs text-gray-400'>{holder.percentage.toFixed(2)}%</div>
															</div>
														</div>
													))}
												</div>
											...
											<div className='p-3 bg-green-900/20 border-2 border-green-600/30'>
												<div className='flex items-start gap-2'>
													...
													<div className='font-mono text-xs text-green-400'>
														<strong>Real Analytics:</strong> Real-time analysis data from on-chain sources. Includes distribution analysis, risk assessment, and liquidity metrics calculated from token accounts and holder patterns.
													</div>
												</div>
											</div>
										</div>
											...
											<div className='p-3 bg-blue-600/10 border-4 border-blue-600/20'>
												<div className='flex items-start gap-2'>
													...
													<div className='font-mono text-xs text-blue-400'>
														<strong>Note:</strong> Advanced analytics data is currently simulated. Real-time data integration with Jupiter, Birdeye, and other price APIs will be available in future versions.
													</div>
												</div>
											</div>
										</div>
				...
				{!tokenInfo && (
					<PixelCard>
						<div className='space-y-4'>
							<div className='border-b-4 border-green-400/20 pb-3'>
								<h3 className='font-pixel text-sm text-green-400'>🚀 FEATURES PREVIEW</h3>
							</div>
